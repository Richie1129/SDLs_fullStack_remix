		...
		<div className="min-w-full min-h-screen h-screen">
							...
							{isLoading ? (
								...
								showEmptyMessage && (
									<div className="flex flex-col items-center justify-center px-4 sm:px-8 lg:px-16">
										<Lottie
											className="w-48 sm:w-56 md:w-64 max-w-full h-auto"
								...
								<div className="relative overflow-hidden w-full py-4 sm:py-6 lg:py-8">
									...
									>
										{personalDaily.map((item, index) => (
											...
											>
												<div
													className="bg-white rounded-lg shadow-lg p-3 sm:p-4 lg:p-6 m-1 sm:m-2 w-full h-full flex flex-col min-h-[400px] sm:min-h-[450px] lg:min-h-[500px]"
													key={index}
												>
														...
														{is5RsFormat(item.content) ? (
															<div className="space-y-2">
																<div className="flex items-center justify-between">
																	...
																	{(() => {
																		const parsed = parse5RsContent(
																			item.content
																		);
																		const hasAIFeedback =
																			parsed?.feedback &&
																			(parsed.feedback.overall ||
																				parsed.feedback.suggestions?.length >
																					0);
														...
														<div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-3">
															...
															{item.updatedAt &&
																item.updatedAt !== item.createdAt && (
																	...
																	>
																		更新:{" "}
																		{formatTime(item.updatedAt, "relative")}
																	</p>
																)}
														</div>
														{/* 5Rs 反思的 AI 分析按鈕 */}
											...
											>
												...
												>
													...
													<div className="flex-grow overflow-auto mb-4 px-2 sm:px-4">
														<p className="text-gray-700 break-words text-sm sm:text-base leading-relaxed">
															{item.content}
														</p>
													</div>
			...
			>
				...
				<div className="flex flex-col px-2 sm:px-4 lg:px-6 py-2 sm:py-4">
					...
					/>
					<textarea
						className="rounded outline-none ring-2 ring-customgreen w-full mb-3 p-1 resize-none overflow-auto"
						rows={10}
						placeholder="撰寫您的日誌..."
						name="content"
						value={content}

 ...
 *
 * Props:
 *   reflections  — 全班反思陣列（包含 content 欄位）
 */
const ClassFiveRsRadarChart = ({ reflections = [] }) => {
  const { chartData, analyzedCount, analyzedStudentCount, totalCount, overall, strongest, weakest } =
    useMemo(() => {
      ...
      const analyzedStudentIds = new Set();
      let analyzed = 0;
      ...
      reflections.forEach(r => {
        ...
        // 追蹤已分析的學生人數
        const uid = r.userId || r.user_id || r.username;
        if (uid) analyzedStudentIds.add(uid);
      ...
      const avgScores = DIMENSIONS.map(({ key, label, fullLabel }) => {
        const avg = scoreCounts[key] > 0
          ? Math.round(scoreSums[key] / scoreCounts[key])
          : 0;
      ...
      return {
        ...
        analyzedStudentCount: analyzedStudentIds.size,
      ...
      {noData ? (
        ...
        <div className="text-center py-10 text-gray-400">
          ...
          <p className="text-caption text-gray-400 max-w-xs mx-auto leading-relaxed">
            {totalCount > 0
              ? '已有反思記錄，請引導學生對 5Rs 反思執行 AI 分析，全班各維度平均分數將在此呈現。'
              : '當學生開始提交 5Rs 反思並取得 AI 分析後，全班各層次的學習深度分布將在此顯示。'}
          </p>
        </div>
      ) : (
        <>
          ...
          {strongest && weakest && strongest.key !== weakest.key && (
            <div className="mt-4 grid grid-cols-2 gap-3">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-component-sm">
                <p className="text-caption text-amber-500 mb-0.5">建議加強教學</p>
                <p className="text-body-sm font-semibold text-amber-700">{weakest.fullLabel}</p>
                <p className="text-caption text-amber-600">{weakest.score} 分 · 建議在課堂強化此層次</p>
              </div>
            </div>
          )}
        </>
...
export default ClassFiveRsRadarChart;

      ...
      {ideaWallChats && ideaWallChats.length > 0 && (
        <div style={{ marginBottom: '8mm' }}>
          ...
          {ideaWallChats.map((iw, i) => (
            <div key={i} style={{ marginBottom: '5mm', border: '1px solid #d0d7e3', borderRadius: '4px', overflow: 'hidden', breakInside: 'avoid' }}>
                ...
                {iw.messages.filter(m => !m.isAiIntervention).length > 0 && (
                  <div>
                    ...
                    {iw.messages.filter(m => !m.isAiIntervention).slice(0, 6).map((m, j) => (
                      <div key={j} style={{ marginBottom: '2mm', paddingLeft: '3mm', borderLeft: '2px solid #c8d4e8', fontSize: '9pt', color: '#333', breakInside: 'avoid' }}>
...
function SectionTitle({ children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '3mm', marginBottom: '5mm', marginTop: '8mm', breakAfter: 'avoid' }}>
...
function StageSection({ stage }) {
  ...
  return (
    <div style={{ marginBottom: '10mm' }}>
      <div style={{ background: '#1e3a5f', color: '#fff', padding: '3mm 6mm', borderRadius: '4px 4px 0 0', fontSize: '11pt', fontWeight: '700', breakAfter: 'avoid' }}>
...
function SubTitle({ children }) {
  return <div style={{ fontSize: '9pt', fontWeight: '700', color: '#555', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '3mm', breakAfter: 'avoid' }}>{children}</div>;
}

function ReflectionCard({ reflection }) {
  return (
    <div style={{ marginBottom: '4mm', padding: '4mm 5mm', background: '#fafbfd', border: '1px solid #e8edf5', borderRadius: '4px', breakInside: 'avoid' }}>

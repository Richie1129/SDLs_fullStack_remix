...
const ProjectCommentDrawer = ({ projectId, isOpen, onClose }) => {
  ...
  const handleAttachmentDownload = async (attachment) => {
    try {
      ...
      const resp = await fetch(`https://science.sdlswuret.com/api/file/download/${fileName}`);
      const data = await resp.json().catch(() => ({}));
      const url = data?.downloadUrl || `https://science.sdlswuret.com/api/file/direct/${fileName}`;
          ...
          <div className="grid grid-cols-3 gap-2">
            {imgs.map(a => (
              <div key={a.id} className="relative group">
                <img
                  src={`https://science.sdlswuret.com/api/file/image/${a.fileName}`}
                  alt={a.originalName}
          ...
          <div className="space-y-1">
            {files.map(a => (
              <div key={a.id} className="flex items-center px-3 py-2 border rounded-md text-sm hover:shadow-sm hover:border-gray-300">
                ...
                <a
                  href={`https://science.sdlswuret.com/api/file/direct/${a.fileName}`}
                  target="_blank"
                  rel="noreferrer"

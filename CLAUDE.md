...
**任何修復或重構，最重要的前提是不能影響現有資料。** 在進行 bug 修復、安全加固、資料庫遷移、欄位變更等操作時：
- 必須確保現有資料的完整性與可存取性不受影響
- 新增驗證邏輯時，須考慮既有資料可能不符合新規則的情況，做好向下相容
- 涉及資料庫 schema 變更時，必須提供可回滾的 migration，且不得刪除或覆蓋既有欄位資料
- 修改 API 回應格式或欄位時，確認前端所有使用處都已同步調整
- 若不確定修復是否會影響資料，先在本地環境驗證，並告知使用者風險

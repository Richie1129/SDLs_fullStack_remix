...
router.get('/status/:projectId', validateToken, checkTeacherRole, requireProjectMentor, getCooldownStatus);

// GET /api/teacher-agent/history/:projectId — 取得過去分析紀錄
router.get('/history/:projectId', validateToken, checkTeacherRole, requireProjectMentor, getHistory);

// POST /api/teacher-agent/analyze/:projectId — 啟動班級分析（SSE 串流）
router.post('/analyze/:projectId', validateToken, checkTeacherRole, requireProjectMentor, startAnalysis);

...
// Helper: derive projectId from attachment id
const getProjectIdFromAttachment = async (req, res, next) => {
  try {
    const { attachmentId } = req.params;
    const att = await ProjectCommentAttachment.findByPk(attachmentId);
    if (!att) return res.status(404).json({ message: '附件不存在' });
    const comment = await ProjectComment.findByPk(att.commentId);
    ...
    console.error('從 attachment 獲取 projectId 錯誤:', error);
    return res.status(500).json({ message: '獲取專案資訊時發生錯誤', error: error.message });
  }
};
...
router.delete(
  '/project-comments/attachments/:attachmentId',
  validateToken,
  getProjectIdFromAttachment,
  checkProjectViewingPermission,
